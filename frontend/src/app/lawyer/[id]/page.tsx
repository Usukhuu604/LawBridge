...
import { Mail, Star, CheckCircle, XCircle, User, ChevronDown } from "lucide-react";
...
const LawyerProfile = ({ params }: Props) => {
  ...
  const [createAppointment, { loading: creatingAppointment }] = useMutation(CREATE_APPOINTMENT, {
    ...
    onError: (error) => {
      setAppointmentStatus({
        type: "error",
        message: error.message || "Цаг захиалга үүсгэхэд алдаа гарлаа. Дахин оролдоно уу.",
      });
    },
  });
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-none sm:rounded-2xl border-0 sm:border border-gray-200 shadow-none sm:shadow-sm p-6 sm:p-8 max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2">Алдаа гарлаа</h3>
            <p className="text-sm sm:text-base text-gray-600 mb-4">Хуульчийн мэдээлэл олдсонгүй. Дахин оролдоно уу.</p>
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-none sm:rounded-2xl border-0 sm:border border-gray-200 shadow-none sm:shadow-sm p-6 sm:p-8 max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2">Нэвтрэх шаардлагатай</h3>
            <p className="text-sm sm:text-base text-gray-600">Та нэвтэрч орсны дараа цаг захиалах боломжтой.</p>
          </div>
        </div>
      </div>
    );
  ...
  console.log("Date range:", today.toDateString(), "to", twoWeeksFromToday.toDateString());
        ...
        <div className="bg-white rounded-none sm:rounded-2xl border-0 sm:border border-gray-200 shadow-none sm:shadow-sm overflow-hidden">
          ...
          <div>
            <div className="p-4 sm:p-6 lg:p-8">
                    ...
                    <div className="space-y-2 text-gray-600">
                      {lawyer.university && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm sm:text-base font-medium text-gray-700">Их сургууль:</span>
                          <span className="text-sm sm:text-base text-gray-600">{lawyer.university}</span>
                        </div>
                      )}
                      <div className="flex gap-2 justify-start items-center">
                        <Mail className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500" />
                        <a href={`mailto:${lawyer.email}`} className="hover:text-[#003366] transition-colors text-sm sm:text-base">
                          {lawyer.email}
                        </a>
                      </div>
                      <button
              ...
              {lawyer.bio && <p className="text-sm sm:text-base text-gray-700 leading-relaxed mt-4 max-w-2xl">{lawyer.bio}</p>}
            </div>
          </div>
                  ...
                  <div className="rounded-none sm:rounded-2xl p-4 sm:p-6 lg:p-8 border-0 sm:border border-gray-100 shadow-none sm:shadow-sm bg-white">
                    <div className="text-center space-y-4 sm:space-y-6">
                      <div className="space-y-2">
                        <h2 className="text-lg sm:text-xl font-bold text-gray-900">Үнэлгээ</h2>
                        <p className="text-sm sm:text-base text-gray-600">
                          Нийт <span className="font-bold text-[#003366]">{lawyer.reviews || 0}</span> үнэлгээ
                        </p>
                      </div>
                      ...
                      <div className="inline-flex items-center bg-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex items-center gap-3">
                          <div className="flex text-yellow-400">
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} className={`h-4 w-4 sm:h-5 sm:w-5 ${i < Math.floor(lawyer.rating) ? "fill-current" : ""}`} />
                            ))}
                          </div>
                          <span className="text-lg sm:text-2xl font-bold text-[#003366]">{lawyer.rating}</span>
                        </div>
                      </div>

                      <p className="text-sm sm:text-base text-gray-500">Үйлчлүүлэгчдийн дундаж үнэлгээ</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="space-y-4 sm:space-y-6 rounded-xl">
                <div className="text-center">
                  ...
                  >
                    <h3 className="font-bold text-lg sm:text-xl text-white">Цаг захиалах</h3>
                    <ChevronDown
                ...
                {showBookingSection && (
                  <div className="transition-all duration-500 ease-in-out">
                    ...
                    <div className="mb-6 sm:mb-8">
                      <div className=" ">
                        <div className="flex items-center gap-2 mb-3">
                          <label className="text-base sm:text-lg font-semibold text-gray-900">Мэргэжлийн чиглэл сонгох</label>
                        </div>

                        {specializationsLoading ? (
                          <div className="flex items-center gap-2 py-3">
                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#003366]"></div>
                            <span className="text-gray-600">Мэргэжлүүдийг ачааллаж байна...</span>
                          </div>
                        ) : specializationsData?.getSpecializationsByLawyer && specializationsData.getSpecializationsByLawyer.length > 0 ? (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {specializationsData.getSpecializationsByLawyer.map((spec) => (
                              <button
                                key={spec._id}
                                onClick={() => setSelectedSpecializationId(spec.specializationId)}
                                className={`group p-4 border-2 rounded-md sm:rounded-xl text-left transition-all duration-200 ${
                                  selectedSpecializationId === spec.specializationId
                                  ...
                                  <div className="flex-1">
                                    <h4
                                      className={`font-semibold text-sm sm:text-base mb-1 ${
                                        selectedSpecializationId === spec.specializationId ? "text-white" : "text-gray-900"
                                      }`}
                                    >
                                      {spec.categoryName || spec.specializationId}
                                    </h4>
                                    {(spec.pricePerHour || spec.pricePerHour === 0) && (
                                      <div
                                        className={`flex items-center gap-1 text-xs sm:text-sm ${
                                          selectedSpecializationId === spec.specializationId ? "text-blue-100" : "text-gray-600"
                                        }`}
                                      >
                                        <span className="font-medium">Үнэ:</span>
                                        <span>{spec.pricePerHour === 0 ? "Үнэгүй" : `${spec.pricePerHour.toLocaleString()}₮/цаг`}</span>
                                      </div>
                                    )}
                                  </div>
                          ...
                          <div className="text-center py-4 ">
                            <p className="text-gray-500">Мэргэжлийн чиглэл байхгүй байна</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {appointmentStatus.type && (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-full ${appointmentStatus.type === "success" ? "bg-green-100" : "bg-red-100"}`}>
                            {appointmentStatus.type === "success" ? <CheckCircle className="h-5 w-5" /> : <XCircle className="h-5 w-5" />}
                          </div>
                          <div>
                            <p className="font-semibold text-sm sm:text-base">{appointmentStatus.message}</p>
                            <p className="text-xs sm:text-sm opacity-75">
                              {appointmentStatus.type === "success" ? "Та цаг захиалгаа амжилттай хийлээ." : "Дахин оролдож үзнэ үү."}
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    {availabilityLoading ? (
                      <div className="text-center py-6 sm:py-8">
                        ...
                        <p className="mt-2 text-gray-600 text-sm sm:text-base">Боломжит цагуудыг ачааллаж байна...</p>
                      </div>
                            ...
                            >
                              <div className="bg-gradient-to-r from-[#003366] to-[#004080] px-4 sm:px-6 py-4 border-b border-gray-200">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    <h4 className="font-bold text-white text-base sm:text-lg">{formatDate(date)}</h4>
                                  </div>
                                  <div className="bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full">
                                    <span className="text-white text-xs sm:text-sm font-medium">{slots.length} цаг</span>
                                  </div>
                                </div>
                              </div>
                                    ...
                                    .map((slot, idx) => (
                                      ...
                                      >
                                        <div className="flex flex-col items-center gap-1 text-[#003366] group-hover:text-white">
                                          <div className="text-xs sm:text-sm font-bold">{formatTime(slot.startTime)}</div>
                                          <div className="text-xs opacity-75">{formatTime(slot.endTime)}</div>
                                        </div>
                      ...
                      <div className="text-center py-12 sm:py-16">
                        <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-md sm:rounded-2xl p-8 sm:p-12 border-2 border-dashed border-gray-300">
                          ...
                          <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2 sm:mb-3">Боломжит цаг байхгүй</h3>
                          <p className="text-gray-600 text-sm sm:text-base mb-4 max-w-md mx-auto">
                            Одоогоор энэ хуульчаас захиалах боломжтой цаг байхгүй байна. Та дараа дахин шалгаж үзээрэй.
                          </p>
                          <div className="flex flex-col sm:flex-row gap-2 justify-center text-xs sm:text-sm text-gray-500">
                            <span>Зөвлөгөө: Хуульч өөрийн цагийн хуваарийг тодорхойлсны дараа цаг харагдана</span>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        {showBookingModal && selectedSlot && (
          <div className="fixed inset-0 bg-opacity-40 backdrop-blur-md flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-md sm:rounded-2xl max-w-md w-full p-4 sm:p-6 max-h-[90vh] overflow-y-auto border border-[#003366]/30 shadow-[0_8px_32px_0_rgba(0,51,102,0.24)]">
              <h3 className="text-lg sm:text-xl font-bold mb-4">Цаг захиалга баталгаажуулах</h3>

              <div className="space-y-4 mb-6">
                <div className="bg-blue-50 p-3 sm:p-4 rounded-md sm:rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-medium text-sm sm:text-base">Огноо: {formatDate(selectedSlot.day)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm sm:text-base">
                      Цаг: {formatTime(selectedSlot.startTime)} - {formatTime(selectedSlot.endTime)}
                    </span>
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Нэмэлт тэмдэглэл (заавал биш)</label>
                  <textarea
