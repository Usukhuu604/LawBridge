...
export default function BookAppointmentModal({
  ...
  const { data, loading } = useQuery<{ getAvailability: AvailabilitySlot[] }>(
    GET_AVAILABILITY,
    { variables: { lawyerId } }
  );
  const [createAppointment, { loading: creating }] = useMutation(
    ...
    {
      onCompleted: () => {
        // Refresh the availability data after creating appointment
        // The query will automatically refetch due to cache updates
      },
      refetchQueries: ["GetAvailability"], // Refetch availability data
    }
  );
